 ...
 *
 * @brief Defines a base C-SVM used for the different SYCL backends.
 */
 ...
 */
class csvm : public ::plssvm::detail::gpu_csvm<::plssvm::sycl::detail::device_ptr, ::plssvm::sycl::detail::queue> {
  protected:
    // protected for the test MOCK class
    /// The template base type of the SYCL C-SVM class.
    using base_type = ::plssvm::detail::gpu_csvm<::plssvm::sycl::detail::device_ptr, ::plssvm::sycl::detail::queue>;
  ...
  protected:
    /**
     * @brief Return a SYCL implementation specific compiler string.
     * @return the SYCL implementation specific string (`[[nodiscard]]`)
     */
    [[nodiscard]] virtual std::string compiler_info() const { return "WONT WORK"; } // TODO: this won't work
