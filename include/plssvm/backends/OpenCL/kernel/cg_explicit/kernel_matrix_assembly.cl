...
__kernel void device_kernel_assembly(__global real_type *kernel_matrix, const __global real_type *data, const ulong num_rows, const ulong device_num_rows, const ulong device_row_offset, const ulong num_features, const __global real_type *q, const real_type QA_cost, const real_type cost, const ulong grid_x_offset, const ulong grid_y_offset PLSSVM_OPENCL_KERNEL_FUNCTION_PARAMETER_LIST) {
    ...
    __local real_type data_i_cache[THREAD_BLOCK_SIZE][THREAD_BLOCK_SIZE];
    __local real_type data_j_cache[THREAD_BLOCK_SIZE][THREAD_BLOCK_SIZE];
    ...
    if (blockIdx_x >= blockIdx_y) {
...

        {
            ...
            const ulong global_i_idx_linear = device_row_offset + blockIdx_x * blockDim_x + threadIdx_x;  // num_rows - device_row_offset
            const ulong global_j_idx_linear = device_row_offset + blockIdx_y * blockDim_y + threadIdx_x;  // device_num_rows
        ...
        const ulong device_global_i_idx = blockIdx_x * blockDim_x + threadIdx_x;
        const ulong global_i_idx = device_row_offset + device_global_i_idx;
        const ulong device_global_j_idx = blockIdx_y * blockDim_y + threadIdx_y;
