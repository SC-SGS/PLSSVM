...
template void device_kernel_q_linear(std::vector<float> &, const detail::aos_matrix<float> &);
template void device_kernel_q_linear(std::vector<double> &, const detail::aos_matrix<double> &);

template <typename real_type>
void device_kernel_q_polynomial(std::vector<real_type> &q, const detail::aos_matrix<real_type> &data, const int degree, const real_type gamma, const real_type coef0) {
    PLSSVM_ASSERT(q.size() == data.num_rows() - 1, "Sizes mismatch!: {} != {}", q.size(), data.num_rows() - 1);
...
template void device_kernel_q_polynomial(std::vector<float> &, const detail::aos_matrix<float> &, int, float, float);
template void device_kernel_q_polynomial(std::vector<double> &, const detail::aos_matrix<double> &, int, double, double);

template <typename real_type>
void device_kernel_q_rbf(std::vector<real_type> &q, const detail::aos_matrix<real_type> &data, const real_type gamma) {
    PLSSVM_ASSERT(q.size() == data.num_rows() - 1, "Sizes mismatch!: {} != {}", q.size(), data.num_rows() - 1);
    PLSSVM_ASSERT(gamma > real_type{ 0.0 }, "gamma must be greater than 0, but is {}!", gamma);
...
template void device_kernel_q_rbf(std::vector<float> &, const detail::aos_matrix<float> &, float);
template void device_kernel_q_rbf(std::vector<double> &, const detail::aos_matrix<double> &, double);
