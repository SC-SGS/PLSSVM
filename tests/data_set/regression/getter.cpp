...
template <typename T>
class RegressionDataSetGetter : public ::testing::Test,
...
TYPED_TEST(RegressionDataSetGetter, data) {
    ...
    const plssvm::regression_data_set<label_type> data{ this->get_data_points() };
    // check data getter
...
TYPED_TEST(RegressionDataSetGetter, has_labels) {
    ...
    // create data set without labels
    const plssvm::regression_data_set<label_type> data_without_labels{ this->get_data_points() };
    // check has_labels getter
    EXPECT_FALSE(data_without_labels.has_labels());
    // create data set with labels
    const plssvm::regression_data_set<label_type> data_with_labels{ this->get_data_points(), this->get_label() };
    // check has_labels getter
...
TYPED_TEST(RegressionDataSetGetter, is_scaled) {
    ...
    const plssvm::regression_data_set<label_type> data_scaled{ this->get_data_points(), scaling_type{ plssvm::real_type{ -1.0 }, plssvm::real_type{ 1.0 } } };
    // check is_scaled getter
