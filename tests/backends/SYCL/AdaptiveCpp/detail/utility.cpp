...
class AdaptiveCppDetailUtility : public ::testing::TestWithParam<std::tuple<::sycl::range<2>, plssvm::sycl::kernel_invocation_type, ::sycl::nd_range<2>>> {
  public:
    ...
    /**
     * @brief The correct grid size when using work_group data parallel kernels.
...
INSTANTIATE_TEST_SUITE_P(AdaptiveCppDetailUtility, AdaptiveCppDetailUtility, ::testing::Values(
        std::make_tuple(::sycl::range<2>{ 42, 42 }, plssvm::sycl::kernel_invocation_type::work_group, ::sycl::nd_range<2>{ AdaptiveCppDetailUtility::correct_grid_work_group(42, 42), AdaptiveCppDetailUtility::correct_block() }),
