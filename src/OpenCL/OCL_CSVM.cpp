...
#pragma omp parallel for
    ...
    for (int device = 0; device < count_devices; ++device)
        data_cl.emplace_back(opencl::DevicePtrOpenCL<real_t>(devices[device], num_features * (num_data_points - 1)));
    ...
    if (info) {
        std::clog << std::endl
                  << data.size() << " Datenpunkte mit Dimension " << num_features << " in " << std::chrono::duration_cast<std::chrono::milliseconds>(end_transform - begin_transform).count() << " ms transformiert" << std::endl;
    }
#pragma omp parallel for
    for (int device = 0; device < count_devices; ++device) {
        data_cl[device] = opencl::DevicePtrOpenCL<real_t>(devices[device], num_features * (num_data_points - 1 + THREADBLOCK_SIZE * INTERNALBLOCK_SIZE));
