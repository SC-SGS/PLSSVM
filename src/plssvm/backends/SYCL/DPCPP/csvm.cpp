    ...
    for (const auto &[partial_grid, offsets] : exec.grids) {
        ...
        switch (params.kernel_type) {
            ...
            case kernel_function_type::laplacian:
                ...
                //     cgh.parallel_for<ass_lap>(native_exec, functor_type{ cgh, kernel_matrix_d.get(), data_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, q_red_d.get(), QA_cost, cost_factor, offsets_ref.y, offsets_ref.x, std::get<real_type>(params.gamma) });
                // });
                break;
...
class device_kernel_symm_explicit;
class device_kernel_symm_explicit_mirror;
