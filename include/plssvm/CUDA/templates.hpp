...
// CUDA memcpy peer
template <typename T, typename D1, typename D2, std::enable_if_t<std::is_integral<D1>::value,
                                                                 int> = 0, std::enable_if_t<std::is_integral<D2>::value,
                                                                                            int> = 0>
inline void cuda_memcpy(std::vector<T>& vec_dst,
...
template <typename T, typename S, typename D1, typename D2, std::enable_if_t<std::is_integral<S>::value,
                                                                             int> = 0, std::enable_if_t<std::is_integral<
    D1>::value, int> = 0, std::enable_if_t<std::is_integral<D2>::value, int> = 0>
inline void cuda_memcpy(std::vector<T>& vec_dst,
                        const D1 device_dest,
                        const std::vector<T>& vec_src,
                        const D2 device_src,
