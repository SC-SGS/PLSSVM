...
class csvm : virtual public ::plssvm::csvm {
  public:
     ...
     * @details Calls the base class move-assignment operator. Afterwards, moves the additional `hpx::csvm` members.
...
/**
 * @brief Create a C-SVC using the HPX backend.
 * @details Inherits all functionality either from the `plssvm::csvc` or `plssvm::hpx::csvm` classes.
 */
...
/**
 * @brief Create a C-SVR using the HPX backend.
 * @details Inherits all functionality either from the `plssvm::csvr` or `plssvm::hpx::csvm` classes.
 */
...
/**
 * @brief Sets the `value` to `true` since C-SVMs (C-SVCs, C-SVRs) using the HPX backend are available.
 */
template <typename T>
struct csvm_backend_exists<T, std::enable_if_t<is_one_type_of_v<T, hpx::csvm, hpx::csvc, hpx::csvr>>> : std::true_type { };
