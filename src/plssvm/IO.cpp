...
void CSVM::libsvmParser(const std::string &filename) {
...

    {
        ...
        while (std::getline(file, line)) {
            std::string_view trimmed = detail::trim_left(line);
            if (!trimmed.empty() && !detail::starts_with(trimmed, '#')) {
...
void CSVM::arffParser(const std::string &filename) {
    ...
    std::size_t max_size = 0;
    {
        ...
        while (std::getline(file, line)) {
            ...
            if (trimmed.empty() || detail::starts_with(trimmed, '%')) {
                ...
                trimmed = detail::trim_left(line);
                if (detail::starts_with(trimmed, "@RELATION")) {
                    // ignore relation
                    continue;
                } else if (detail::starts_with(trimmed, "@ATTRIBUTE")) {
                    ...
                    // add a feature
                    ++max_size;
                } else if (detail::starts_with(trimmed, "@DATA")) {
        ...
        while (std::getline(file, line)) {
            ...
            if (trimmed.empty() || detail::starts_with(trimmed, '%')) {
                // ignore empty lines or comments
                continue;
            } else if (detail::starts_with(trimmed, '@')) {
    ...
    #pragma omp parallel
    {
        ...
        for (std::size_t i = 0; i < data.size(); ++i) {
            #pragma omp cancellation point for
            try {
                std::string_view line{ detail::trim_left(data_lines[i]) };
