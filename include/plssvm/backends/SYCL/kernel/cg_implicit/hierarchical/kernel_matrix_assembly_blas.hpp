...
class device_kernel_assembly_symm {
  public:
    ...
    void operator()(::sycl::group<2> group) const {
        ...
        std::array<real_type, static_cast<std::size_t>(THREAD_BLOCK_SIZE) * static_cast<std::size_t>(INTERNAL_BLOCK_SIZE) * static_cast<std::size_t>(THREAD_BLOCK_SIZE)> cache_one{};
        std::array<real_type, static_cast<std::size_t>(THREAD_BLOCK_SIZE) * static_cast<std::size_t>(INTERNAL_BLOCK_SIZE) * static_cast<std::size_t>(THREAD_BLOCK_SIZE)> cache_two{};
