...
try:
    ...
    plssvm.detail.tracking.PerformanceTracker.pause()
    ...
    plssvm.detail.tracking.PerformanceTracker.resume()
    ...
    for backend in available_backends:
        ...
        for rep in range(args.num_repeats):
            ...
            # create the data set
            train_data = plssvm.ClassificationDataSet(args.intermediate_train_file)

            # create a default C-SVM
            svm = plssvm.CSVC(backend=backend[0], **backend[1])
            ...
            plssvm.detail.tracking.PerformanceTracker.add_parameter_tracking_entry(params)
            plssvm.detail.tracking.PerformanceTracker.add_string_tracking_entry("", "accuracy", str(achieved_accuracy))
            plssvm.detail.tracking.PerformanceTracker.add_string_tracking_entry("", "total_runtime", "{}ms".format(
                int((end_time - start_time) * 1000)))
            plssvm.detail.tracking.PerformanceTracker.save(args.performance_tracking)
