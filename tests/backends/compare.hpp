...
template <plssvm::kernel_type kernel, typename real_type, typename SVM>
std::vector<real_type> device_kernel_function(const std::vector<std::vector<real_type>> &data, std::vector<real_type> &x, const std::vector<real_type> &q, real_type QA_cost, real_type cost, int sgn, [[maybe_unused]] const SVM &csvm) {
