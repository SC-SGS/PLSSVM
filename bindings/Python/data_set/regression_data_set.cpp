...
void init_regression_data_set(py::module_ &m) {
    ...
    py::class_<regression_data_set_wrapper>(m, "RegressionDataSet", "Encapsulate all necessary data that is needed for training or predicting using an C-SVR.")
        ...
        .def(py::init([](plssvm::aos_matrix<plssvm::real_type> data, const std::optional<py::type> &type, const std::optional<plssvm::min_max_scaler> &scaler, plssvm::mpi::communicator comm) {
             ...
             py::arg("scaler") = std::nullopt,
             py::arg("comm") = plssvm::mpi::communicator{})
        .def(py::init([](plssvm::aos_matrix<plssvm::real_type> data, plssvm::bindings::python::util::label_vector_wrapper<typename regression_data_set_wrapper::possible_vector_types> labels, const std::optional<plssvm::min_max_scaler> scaler, plssvm::mpi::communicator comm) {
