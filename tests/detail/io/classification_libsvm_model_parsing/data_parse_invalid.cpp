 ...
 *
 * @brief Tests for parsing an invalid LIBSVM classification model file data section.
 */
...
template <typename T>
class LIBSVMClassificationModelDataParseInvalid : public ::testing::Test { };
...
template <typename T>
class LIBSVMClassificationModelDataParseInvalidDeathTest : public LIBSVMClassificationModelDataParseInvalid<T> { };
...
TYPED_TEST(LIBSVMClassificationModelDataParseInvalidDeathTest, too_few_num_sv_per_class) {
    ...
    EXPECT_DEATH(std::ignore = (plssvm::detail::io::parse_libsvm_model_data_classification(reader, std::vector<std::size_t>{ 3 }, 0)),
...
TYPED_TEST(LIBSVMClassificationModelDataParseInvalidDeathTest, skip_too_many_lines) {
    ...
    reader.read_lines('#');
    // try to skip more lines than are present in the data file
    EXPECT_DEATH(std::ignore = (plssvm::detail::io::parse_libsvm_model_data_classification(reader, std::vector<std::size_t>{ 3, 3 }, 15)),
