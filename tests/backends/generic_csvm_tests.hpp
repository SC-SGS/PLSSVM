...
/**
 * @brief Initialize the matrix using the values provided by @p matr based on the @p csvm_type.
 * @details Returns a `plssvm::detail::simple_any` with a value based on the @p csvm_type.
 * @tparam csvm_type the CSVM backend type
 ...
 */
template <typename csvm_type, typename device_ptr_type, typename T>
[[nodiscard]] inline auto init_matrix(std::vector<T> matr) {
...
TYPED_TEST_P(GenericCSVMSolver, setup_data_on_devices) {
    ...
    // minimal example
    const auto input = util::generate_specific_matrix<plssvm::aos_matrix<plssvm::real_type>>(3, 3);
    ...
    if constexpr (solver == plssvm::solver_type::automatic) {
        ...
        if constexpr (plssvm::csvm_to_backend_type_v<csvm_type> == plssvm::backend_type::openmp) {
            ...
            EXPECT_FLOATING_POINT_VECTOR_EQ(data, util::flatten(input.to_2D_vector()));
        }
    }
}
...
#endif
    const plssvm::detail::simple_any A{ util::init_matrix<csvm_type, device_ptr_type>(matr_A) };
...
#if defined(PLSSVM_USE_GEMM)
    const std::vector<plssvm::real_type> matr_A{ plssvm::real_type{ 1.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 1.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 1.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 1.0 } };
#else
...
TYPED_TEST_P(GenericCSVMSolver, conjugate_gradients) {
    ...
    // create the data that should be used
#if defined(PLSSVM_USE_GEMM)
    const std::vector<plssvm::real_type> matr_A{ plssvm::real_type{ 4.0 }, plssvm::real_type{ 1.0 }, plssvm::real_type{ 1.0 }, plssvm::real_type{ 3.0 } };
#else
...
TYPED_TEST_P(GenericCSVMSolverKernelFunction, solve_lssvm_system_of_linear_equations_trivial) {
    ...
    const plssvm::aos_matrix<plssvm::real_type> A{ { { plssvm::real_type{ std::sqrt(plssvm::real_type(1.0) - 1 / params.cost) }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 }, plssvm::real_type{ 0.0 } },
...
#if defined(PLSSVM_USE_GEMM)
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ 2.0 }, static_cast<plssvm::real_type>(std::exp(-27.0)), static_cast<plssvm::real_type>(std::exp(-27.0)), plssvm::real_type{ 2.0 } }));
#else
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ 2.0 }, static_cast<plssvm::real_type>(std::exp(-27.0)), plssvm::real_type{ 2.0 } }));
#endif
        } else {
#if defined(PLSSVM_USE_GEMM)
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ 15.0 }, plssvm::real_type{ 32.0 }, plssvm::real_type{ 32.0 }, plssvm::real_type{ 78.0 } }));
#else
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ 15.0 }, plssvm::real_type{ 32.0 }, plssvm::real_type{ 78.0 } }));
#endif
        }
    }
}
...
#if defined(PLSSVM_USE_GEMM)
                EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ static_cast<plssvm::real_type>(std::pow(14.0 / 3.0 + 1.0, 3) - 3.0), static_cast<plssvm::real_type>(std::pow(32.0 / 3.0 + 1.0, 3) - 5.0), static_cast<plssvm::real_type>(std::pow(32.0 / 3.0 + 1.0, 3) - 5.0), static_cast<plssvm::real_type>(std::pow(77.0 / 3.0 + 1.0, 3) + 1.0 - 6.0) }));
#else
                EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ static_cast<plssvm::real_type>(std::pow(14.0 / 3.0 + 1.0, 3) - 3.0), static_cast<plssvm::real_type>(std::pow(32.0 / 3.0 + 1.0, 3) - 5.0), static_cast<plssvm::real_type>(std::pow(77.0 / 3.0 + 1.0, 3) + 1.0 - 6.0) }));
#endif
                break;
            case plssvm::kernel_function_type::rbf:
#if defined(PLSSVM_USE_GEMM)
                EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ -2.0 }, static_cast<plssvm::real_type>(std::exp(-9.0) - 5.0), static_cast<plssvm::real_type>(std::exp(-9.0) - 5.0), plssvm::real_type{ -4.0 } }));
#else
                EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<plssvm::real_type>{ plssvm::real_type{ -2.0 }, static_cast<plssvm::real_type>(std::exp(-9.0) - 5.0), plssvm::real_type{ -4.0 } }));
#endif
                break;
        }
    }
...
TYPED_TEST_P(GenericCSVMSolverDeathTest, conjugate_gradients_empty_B) {
    ...
    const std::vector<plssvm::real_type> matr_A(24);
    const plssvm::detail::simple_any A{ util::init_matrix<csvm_type, device_ptr_type>(matr_A) };
...
TYPED_TEST_P(GenericCSVMSolverDeathTest, conjugate_gradients_invalid_eps) {
    ...
    const std::vector<plssvm::real_type> matr_A(24);
    const plssvm::detail::simple_any A{ util::init_matrix<csvm_type, device_ptr_type>(matr_A) };
    const plssvm::aos_matrix<plssvm::real_type> B{ 1, 6 };
...
TYPED_TEST_P(GenericCSVMSolverDeathTest, run_blas_level_3_empty_B) {
    ...
    const std::vector<plssvm::real_type> matr_A(16);
    const plssvm::detail::simple_any A{ util::init_matrix<csvm_type, device_ptr_type>(matr_A) };
    const plssvm::aos_matrix<plssvm::real_type> empty_matr{};
    plssvm::aos_matrix<plssvm::real_type> C{ 4, 4 };
...
TYPED_TEST_P(GenericCSVMSolverDeathTest, run_blas_level_3_empty_C) {
    ...
    const std::vector<plssvm::real_type> matr_A(16);
    const plssvm::detail::simple_any A{ util::init_matrix<csvm_type, device_ptr_type>(matr_A) };
    const plssvm::aos_matrix<plssvm::real_type> B{ 4, 4 };
...
TYPED_TEST_P(GenericCSVMKernelFunctionDeathTest, predict_values_empty_matrices) {
    ...
    const auto support_vectors = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(4, 4);
    const auto weights = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);
    const std::vector<plssvm::real_type> rho(2);
    plssvm::aos_matrix<plssvm::real_type> w{};
    const auto data = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);

    // support vectors shouldn't be empty
    EXPECT_DEATH(std::ignore = csvm.predict_values(params, empty_matr, weights, rho, w, data), "The support vectors must not be empty!");
    // weights shouldn't be empty
    EXPECT_DEATH(std::ignore = csvm.predict_values(params, support_vectors, empty_matr, rho, w, data), ::testing::HasSubstr("The alpha vectors (weights) must not be empty!"));
    // predict points shouldn't be empty
    EXPECT_DEATH(std::ignore = csvm.predict_values(params, support_vectors, weights, rho, w, empty_matr), ::testing::HasSubstr("The data points to predict must not be empty!"));
}
TYPED_TEST_P(GenericCSVMKernelFunctionDeathTest, predict_values_sv_alpha_size_mismatch) {
    ...
    // create correct input matrices
    const auto support_vectors = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(3, 4);
    const auto weights = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);
    const std::vector<plssvm::real_type> rho(2);
    plssvm::aos_matrix<plssvm::real_type> w{};
    const auto data = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);
...
TYPED_TEST_P(GenericCSVMKernelFunctionDeathTest, predict_values_num_features_mismatch) {
    ...
    // create correct input matrices
    const auto support_vectors = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(4, 5);
    const auto weights = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);
    const std::vector<plssvm::real_type> rho(2);
    plssvm::aos_matrix<plssvm::real_type> w{};
    const auto data = util::generate_random_matrix<plssvm::aos_matrix<plssvm::real_type>>(2, 4);
