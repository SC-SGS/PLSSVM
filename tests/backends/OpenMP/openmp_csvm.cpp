...
TEST_F(OpenMPCSVM, calculate_w) {
    ...
    plssvm::openmp::device_kernel_w_linear(w, weights, data.data());
...
TYPED_TEST(OpenMPCSVMKernelFunction, assemble_kernel_matrix_explicit) {
    ...
    switch (kernel) {
        case plssvm::kernel_function_type::linear:
            plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::linear>(q_red, kernel_matrix, aos_data, QA_cost, cost);
            break;
        case plssvm::kernel_function_type::polynomial:
            plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::polynomial>(q_red, kernel_matrix, aos_data, QA_cost, cost, params.degree.value(), params.gamma.value(), params.coef0.value());
            break;
        case plssvm::kernel_function_type::rbf:
            plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::rbf>(q_red, kernel_matrix, aos_data, QA_cost, cost, params.gamma.value());
            break;
    }
...
TYPED_TEST(OpenMPCSVMKernelFunction, blas_level_3_kernel_implicit) {
    ...
    switch (kernel) {
        case plssvm::kernel_function_type::linear:
            plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::linear>(alpha, q_red, aos_data, QA_cost, cost, aos_B, beta, aos_C);
            break;
        case plssvm::kernel_function_type::polynomial:
            plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::polynomial>(alpha, q_red, aos_data, QA_cost, cost, aos_B, beta, aos_C, params.degree.value(), params.gamma.value(), params.coef0.value());
            break;
        case plssvm::kernel_function_type::rbf:
            plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::rbf>(alpha, q_red, aos_data, QA_cost, cost, aos_B, beta, aos_C, params.gamma.value());
            break;
    }
...
TYPED_TEST(OpenMPCSVMKernelFunction, predict_values) {
    ...
    switch (kernel) {
        case plssvm::kernel_function_type::linear:
            plssvm::openmp::device_kernel_predict_linear(out, correct_w, rho, predict_points);
            break;
        case plssvm::kernel_function_type::polynomial:
            plssvm::openmp::device_kernel_predict<plssvm::kernel_function_type::polynomial>(out, weights, rho, data.data(), predict_points, params.degree.value(), params.gamma.value(), params.coef0.value());
            break;
        case plssvm::kernel_function_type::rbf:
            plssvm::openmp::device_kernel_predict<plssvm::kernel_function_type::rbf>(out, weights, rho, data.data(), predict_points, params.gamma.value());
            break;
    }
...
TEST_F(OpenMPCSVMDeathTest, calculate_w) {
    ...
    EXPECT_DEATH(plssvm::openmp::device_kernel_w_linear(w, weights_wrong, data.data()), fmt::format("Size mismatch: {} vs {}!", weights_wrong.num_cols(), data.data().num_rows()));
    // the w shape must be correct
    plssvm::soa_matrix<plssvm::real_type> w_wrong{};
    EXPECT_DEATH(plssvm::openmp::device_kernel_w_linear(w_wrong, weights, data.data()), ::testing::HasSubstr(fmt::format("Shape mismatch: [0, 0] vs [{}, {}]!", weights.num_rows(), data.data().num_cols())));
}
...
TYPED_TEST(OpenMPCSVMKernelFunctionDeathTest, assemble_kernel_matrix_explicit) {
    ...
    const auto run_assembly = [=](const plssvm::parameter &params_p, const std::vector<plssvm::real_type> &q_red_p, std::vector<plssvm::real_type> &kernel_matrix_p, const plssvm::soa_matrix<plssvm::real_type> &data_p, const plssvm::real_type QA_cost_p) {
        ...
        switch (kernel) {
            case plssvm::kernel_function_type::linear:
                plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::linear>(q_red_p, kernel_matrix_p, aos_data_p, QA_cost_p, params_p.cost.value());
                break;
            case plssvm::kernel_function_type::polynomial:
                plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::polynomial>(q_red_p, kernel_matrix_p, aos_data_p, QA_cost_p, params_p.cost.value(), params_p.degree.value(), params_p.gamma.value(), params_p.coef0.value());
                break;
            case plssvm::kernel_function_type::rbf:
                plssvm::openmp::device_kernel_assembly<plssvm::kernel_function_type::rbf>(q_red_p, kernel_matrix_p, aos_data_p, QA_cost_p, params_p.cost.value(), params_p.gamma.value());
                break;
        }
    };
...
TYPED_TEST(OpenMPCSVMKernelFunctionDeathTest, blas_level_3_kernel_implicit) {
    ...
    const auto run_assembly_symm = [=](const plssvm::parameter &params_p, const std::vector<plssvm::real_type> &q_red_p, const plssvm::soa_matrix<plssvm::real_type> &data_p, const plssvm::aos_matrix<plssvm::real_type> &aos_B_p, plssvm::aos_matrix<plssvm::real_type> &aos_C_p) {
        ...
        switch (kernel) {
            case plssvm::kernel_function_type::linear:
                plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::linear>(alpha, q_red_p, aos_data_p, QA_cost, params_p.cost.value(), aos_B_p, beta, aos_C_p);
                break;
            case plssvm::kernel_function_type::polynomial:
                plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::polynomial>(alpha, q_red_p, aos_data_p, QA_cost, params_p.cost.value(), aos_B_p, beta, aos_C_p, params_p.degree.value(), params_p.gamma.value(), params_p.coef0.value());
                break;
            case plssvm::kernel_function_type::rbf:
                plssvm::openmp::device_kernel_assembly_symm<plssvm::kernel_function_type::rbf>(alpha, q_red_p, aos_data_p, QA_cost, params_p.cost.value(), aos_B_p, beta, aos_C_p, params_p.gamma.value());
                break;
        }
    };
...
TYPED_TEST(OpenMPCSVMKernelFunctionDeathTest, predict_values) {
    ...
    if constexpr (kernel == plssvm::kernel_function_type::linear) {
        ...
        rho_wrong.pop_back();
        EXPECT_DEATH(plssvm::openmp::device_kernel_predict_linear(out, w, rho_wrong, predict_points),
        ...
        EXPECT_DEATH(plssvm::openmp::device_kernel_predict_linear(out, w, rho, predict_points_wrong),
        ...
        EXPECT_DEATH(plssvm::openmp::device_kernel_predict_linear(out_wrong, w, rho, predict_points),
        ...
        const auto run_predict_values = [=](const plssvm::parameter &params_p, plssvm::aos_matrix<plssvm::real_type> &out_p, const plssvm::aos_matrix<plssvm::real_type> &weights_p, const std::vector<plssvm::real_type> &rho_p, const plssvm::soa_matrix<plssvm::real_type> &support_vectors_p, const plssvm::soa_matrix<plssvm::real_type> &predict_points_p) {
            switch (kernel) {
                case plssvm::kernel_function_type::linear:
                    ...
                    break;
                case plssvm::kernel_function_type::polynomial:
                    plssvm::openmp::device_kernel_predict<plssvm::kernel_function_type::polynomial>(out_p, weights_p, rho_p, support_vectors_p, predict_points_p, params_p.degree.value(), params_p.gamma.value(), params_p.coef0.value());
                    break;
                case plssvm::kernel_function_type::rbf:
                    plssvm::openmp::device_kernel_predict<plssvm::kernel_function_type::rbf>(out_p, weights_p, rho_p, support_vectors_p, predict_points_p, params_p.gamma.value());
                    break;
            }
        };
