...
std::vector<double>CSVM::CG(const std::vector<double> &b,const int imax,  const double eps)
{
	...
	for(run = 0; run < imax ; ++run){
		...
		#ifdef WITH_OPENCL
		{
			...
			std::cout << "Ad: ";
			for(double & val : std::vector<double>(Ad, Ad+dept)) std::cout << val << " "; std::cout << std::endl;
		...
		std::cout << "x: ";
			for(double & val :buffer_x) std::cout << val << " "; std::cout << std::endl;
		}
		...
		cudaDeviceSynchronize();
		std::cout << "x: ";
			for(double & val : std::vector<double>(x_d, x_d+dept)) std::cout << val << " "; std::cout << std::endl;
		#else
	...
	#ifdef WITH_OPENCL
	{
		...
		std::cout << "alpha: ";
		for(double & val : alpha ) std::cout << val << " "; std::cout << std::endl;
