...
void csvm::init(const target_platform target) {
    ...
    plssvm::detail::log(verbosity_level::full,
                        ...
                        plssvm::detail::tracking_entry{ "dependencies", "opencl_target_version", detail::get_opencl_target_version() });
    PLSSVM_DETAIL_PERFORMANCE_TRACKER_ADD_TRACKING_ENTRY((plssvm::detail::tracking_entry{ "dependencies", "opencl_library", std::string{ PLSSVM_OPENCL_LIBRARY } }));
    ...
    for (typename std::vector<queue_type>::size_type device = 0; device < devices_.size(); ++device) {
        ...
        PLSSVM_DETAIL_PERFORMANCE_TRACKER_ADD_TRACKING_ENTRY((plssvm::detail::tracking_entry{ "dependencies", "device_driver_version", driver_version }));
    }
...
auto csvm::run_assemble_kernel_matrix_explicit(const std::size_t device_id, const parameter &params, const device_ptr_type &data_d, const device_ptr_type &q_red_d, real_type QA_cost) const -> device_ptr_type {
    ...
    switch (params.kernel_type) {
        ...
        case kernel_function_type::polynomial:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_explicit), grid, block, kernel_matrix_d.get(), data_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, q_red_d.get(), QA_cost, cost_factor, params.degree, get_gamma_value(params.gamma), params.coef0);
            break;
        case kernel_function_type::rbf:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_explicit), grid, block, kernel_matrix_d.get(), data_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, q_red_d.get(), QA_cost, cost_factor, get_gamma_value(params.gamma));
            break;
        case kernel_function_type::sigmoid:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_explicit), grid, block, kernel_matrix_d.get(), data_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, q_red_d.get(), QA_cost, cost_factor, get_gamma_value(params.gamma), params.coef0);
            break;
...
void csvm::run_assemble_kernel_matrix_implicit_blas_level_3(const std::size_t device_id, const real_type alpha, const device_ptr_type &A_d, const parameter &params, const device_ptr_type &q_red, const real_type QA_cost, const device_ptr_type &B_d, device_ptr_type &C_d) const {
    ...
    switch (params.kernel_type) {
        case kernel_function_type::linear:
            ...
            break;
        case kernel_function_type::polynomial:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_implicit_blas), grid, block, alpha, q_red.get(), A_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, QA_cost, cost_factor, B_d.get(), C_d.get(), num_classes, params.degree, get_gamma_value(params.gamma), params.coef0);
            break;
        case kernel_function_type::rbf:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_implicit_blas), grid, block, alpha, q_red.get(), A_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, QA_cost, cost_factor, B_d.get(), C_d.get(), num_classes, get_gamma_value(params.gamma));
            break;
        case kernel_function_type::sigmoid:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_implicit_blas), grid, block, alpha, q_red.get(), A_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, QA_cost, cost_factor, B_d.get(), C_d.get(), num_classes, get_gamma_value(params.gamma), params.coef0);
            break;
        case kernel_function_type::laplacian:
            detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::assemble_kernel_matrix_implicit_blas), grid, block, alpha, q_red.get(), A_d.get(), num_rows_reduced, device_specific_num_rows, row_offset, num_features, QA_cost, cost_factor, B_d.get(), C_d.get(), num_classes, get_gamma_value(params.gamma));
            break;
    ...
    if (params.kernel_type == kernel_function_type::linear) {
        ...
        switch (params.kernel_type) {
            case kernel_function_type::linear:
                ...
                break;
            case kernel_function_type::polynomial:
                detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::predict_kernel_polynomial), grid, block, out_d.get(), alpha_d.get(), rho_d.get(), sv_or_w_d.get(), predict_points_d.get(), num_classes, num_sv, num_predict_points, num_features, params.degree, get_gamma_value(params.gamma), params.coef0);
                break;
            case kernel_function_type::rbf:
                detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::predict_kernel_rbf), grid, block, out_d.get(), alpha_d.get(), rho_d.get(), sv_or_w_d.get(), predict_points_d.get(), num_classes, num_sv, num_predict_points, num_features, get_gamma_value(params.gamma));
                break;
            case kernel_function_type::sigmoid:
                detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::predict_kernel_sigmoid), grid, block, out_d.get(), alpha_d.get(), rho_d.get(), sv_or_w_d.get(), predict_points_d.get(), num_classes, num_sv, num_predict_points, num_features, get_gamma_value(params.gamma), params.coef0);
                break;
            case kernel_function_type::laplacian:
                detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::predict_kernel_laplacian), grid, block, out_d.get(), alpha_d.get(), rho_d.get(), sv_or_w_d.get(), predict_points_d.get(), num_classes, num_sv, num_predict_points, num_features, get_gamma_value(params.gamma));
                break;
            case kernel_function_type::chi_squared:
                detail::run_kernel(device, device.get_kernel(detail::compute_kernel_name::predict_kernel_chi_squared), grid, block, out_d.get(), alpha_d.get(), rho_d.get(), sv_or_w_d.get(), predict_points_d.get(), num_classes, num_sv, num_predict_points, num_features, get_gamma_value(params.gamma));
                break;
        }
    }
    detail::device_synchronize(device);
