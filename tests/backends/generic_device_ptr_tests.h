...
template <typename device_ptr_type>
void test_memcpy_vector_exception() {
    using real_type = typename device_ptr_type::value_type;
    ...
    EXPECT_THROW_WHAT(ptr.memcpy_to_device(data), plssvm::gpu_device_ptr_exception, "Too few data to perform memcpy (needed: 10, provided: 8)!");

    // try copying data back to the host with a buffer with too few elements
    std::vector<real_type> result(8);
    EXPECT_THROW_WHAT(ptr.memcpy_to_host(data), plssvm::gpu_device_ptr_exception, "Buffer too small to perform memcpy (needed: 10, provided: 8)!");
}

template <typename device_ptr_type>
void test_memcpy_vector_with_count() {
    using real_type = typename device_ptr_type::value_type;
...
template <typename device_ptr_type>
void test_memcpy_vector_with_count_exception() {
    using real_type = typename device_ptr_type::value_type;
    ...
    EXPECT_THROW_WHAT(ptr.memcpy_to_device(data, 1, 7), plssvm::gpu_device_ptr_exception, "Too few data to perform memcpy (needed: 5, provided: 4)!");

    // try copying data back to the host with a buffer with too few elements
    std::vector<real_type> result(4);
    EXPECT_THROW_WHAT(ptr.memcpy_to_host(data, 1, 7), plssvm::gpu_device_ptr_exception, "Buffer too small to perform memcpy (needed: 5, provided: 4)!");
}

template <typename device_ptr_type>
void test_memcpy_ptr() {
    using real_type = typename device_ptr_type::value_type;
    ...
    // copy data to the device
    ptr.memcpy_to_device(data.data());
    // copy data back to the host
    std::vector<real_type> result(ptr.size());
    ptr.memcpy_to_host(result.data());

    // check values for correctness
    EXPECT_EQ(result, std::vector<real_type>(10, 42));
}
template <typename device_ptr_type>
void test_memcpy_ptr_death_test() {
    using real_type = typename device_ptr_type::value_type;
    ...
    EXPECT_DEATH(ptr.memcpy_to_device(nullptr), "Invalid pointer for the data to copy!");
    EXPECT_DEATH(ptr.memcpy_to_host(nullptr), "Invalid pointer for the data to copy!");
    ...
    EXPECT_DEATH(def.memcpy_to_device(data), "Invalid data pointer!");
    EXPECT_DEATH(def.memcpy_to_host(data), "Invalid data pointer!");
}

template <typename device_ptr_type>
void test_memcpy_ptr_with_count() {
    using real_type = typename device_ptr_type::value_type;
    ...
    // copy data to the device
    ptr.memcpy_to_device(data.data(), 1, 3);
    ...
    }
    {
        // copy only a view values back from the device
        std::vector<real_type> result(3);
        ptr.memcpy_to_host(result.data(), 2, 3);
        // check values for correctness
        EXPECT_EQ(result, (std::vector<real_type>{ real_type{ 42.0 }, real_type{ 42.0 }, real_type{ 0.0 } }));
    }

    // copy data to the device
    ptr.memcpy_to_device(data.data(), 2, 6);
...
template <typename device_ptr_type>
void test_memcpy_ptr_with_count_death_test() {
    using real_type = typename device_ptr_type::value_type;
    ...
    // memcpy with invalid data pointer
    EXPECT_DEATH(ptr.memcpy_to_device(nullptr, 0, 10), "Invalid pointer for the data to copy!");
    EXPECT_DEATH(ptr.memcpy_to_host(nullptr, 0, 10), "Invalid pointer for the data to copy!");
    ...
    // memcpy with invalid data pointer
    std::vector<real_type> data(def.size());
    EXPECT_DEATH(def.memcpy_to_device(data, 0, 0), "Invalid data pointer!");
    EXPECT_DEATH(def.memcpy_to_host(data, 0, 0), "Invalid data pointer!");
}
