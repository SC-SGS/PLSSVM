...
TYPED_TEST_P(GenericCSVM, setup_data_on_devices) {
    using mock_csvm_type = typename TypeParam::mock_csvm_type;
    using device_ptr_type = typename TypeParam::device_ptr_type;
    using plssvm::real_type;

    // minimal example
    const plssvm::aos_matrix<real_type> input{ std::vector<std::vector<real_type>>{
    ...
    // TODO: other solver types
    const plssvm::detail::simple_any data_d = svm.setup_data_on_devices(plssvm::solver_type::cg_explicit, input);
    auto data = data_d.get<device_ptr_type>();

    // TODO: other backends
    // check returned values
    ASSERT_EQ(data->shape(), (std::array<std::size_t, 2>{ 3, 3 }));
    ASSERT_EQ(data->num_entries(), 9);
    EXPECT_FLOATING_POINT_MATRIX_EQ(*data, input);
}

TYPED_TEST_P(GenericCSVM, assemble_kernel_matrix_minimal) {
    using mock_csvm_type = typename TypeParam::mock_csvm_type;
    using plssvm::real_type;

    const plssvm::parameter params{
        plssvm::kernel_type = TypeParam::kernel_type,
        plssvm::cost = real_type{ 1.0 },
        plssvm::gamma = real_type{ 1.0 },
        plssvm::coef0 = real_type{ 0.0 },
        plssvm::degree = 1 };
    const plssvm::aos_matrix<real_type> data{ std::vector<std::vector<real_type>>{
        { real_type{ 1.0 }, real_type{ 2.0 }, real_type{ 3.0 } },
        { real_type{ 4.0 }, real_type{ 5.0 }, real_type{ 6.0 } },
        { real_type{ 7.0 }, real_type{ 8.0 }, real_type{ 9.0 } } } };
    const std::vector<real_type> q_red(data.num_cols() - 1, real_type{ 0.0 });
    const real_type QA_cost{ 0.0 };
    ...
    // TODO: other solver types
    const plssvm::detail::simple_any data_d = svm.setup_data_on_devices(plssvm::solver_type::cg_explicit, data);
    const plssvm::detail::simple_any kernel_matrix_d = svm.assemble_kernel_matrix(plssvm::solver_type::cg_explicit, params, data_d, q_red, QA_cost);
    auto kernel_matrix = kernel_matrix_d.get<std::vector<real_type>>();
    ...
    if constexpr (TypeParam::kernel_type == plssvm::kernel_function_type::rbf) {
#if defined(PLSSVM_USE_GEMM)
        EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ 2.0 }, real_type{ std::exp(-27.0) }, real_type{ std::exp(-27.0) }, real_type{ 2.0 } }));
#else
        EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ 2.0 }, real_type{ std::exp(-27.0) }, real_type{ 2.0 } }));
#endif
    } else {
#if defined(PLSSVM_USE_GEMM)
        EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ 15.0 }, real_type{ 32.0 }, real_type{ 32.0 }, real_type{ 78.0 } }));
#else
        EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ 15.0 }, real_type{ 32.0 }, real_type{ 78.0 } }));
#endif
    }
}

TYPED_TEST_P(GenericCSVM, assemble_kernel_matrix) {
    using mock_csvm_type = typename TypeParam::mock_csvm_type;
    using plssvm::real_type;

    const plssvm::parameter params{
        plssvm::kernel_type = TypeParam::kernel_type,
        plssvm::coef0 = real_type{ 1.0 },
        plssvm::gamma = real_type{ 1.0 / 3.0 }
    };
    const plssvm::aos_matrix<real_type> data{ std::vector<std::vector<real_type>>{
        { real_type{ 1.0 }, real_type{ 2.0 }, real_type{ 3.0 } },
        { real_type{ 4.0 }, real_type{ 5.0 }, real_type{ 6.0 } },
        { real_type{ 7.0 }, real_type{ 8.0 }, real_type{ 9.0 } } } };
    const std::vector<real_type> q_red(data.num_cols() - 1, real_type{ 0.0 }); // TODO: set
    const real_type QA_cost{ 0.0 }; // TODO: set
...
#if defined(PLSSVM_USE_GEMM)
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ std::pow(14.0 / 3.0 + 1.0, 3) + 1.0 }, real_type{ std::pow(32.0 / 3.0 + 1.0, 3) }, real_type{ std::pow(32.0 / 3.0 + 1.0, 3) }, real_type{ std::pow(77.0 / 3.0 + 1.0, 3) + 1.0 } }));
#else
            EXPECT_FLOATING_POINT_VECTOR_NEAR(kernel_matrix, (std::vector<real_type>{ real_type{ std::pow(14.0 / 3.0 + 1.0, 3) + 1.0 }, real_type{ std::pow(32.0 / 3.0 + 1.0, 3) }, real_type{ std::pow(77.0 / 3.0 + 1.0, 3) + 1.0 } }));
#endif
            break;
...
REGISTER_TYPED_TEST_SUITE_P(GenericCSVM,
                            ...
                            setup_data_on_devices, assemble_kernel_matrix_minimal, assemble_kernel_matrix);
//                            solve_system_of_linear_equations, solve_system_of_linear_equations_with_correction,
//                            predict_values, predict, score);  // solve_system_of_linear_equations_trivial,
// clang-format on
...
//
//    // create parameter struct
//    const plssvm::parameter params{ kernel, 2, 0.001, 1.0, 0.1 };
//
//    // create the data that should be used
...
//    constexpr plssvm::kernel_function_type kernel = TypeParam::kernel_type;
//
//    using device_ptr_type = typename mock_csvm_type::device_ptr_type;
...
//    const std::vector<real_type> ground_truth = compare::calculate_w(support_vectors.data(), weights);  // TODO: weights?!
//
...
////    const real_type QA_cost = compare::kernel_function(params, data.data(), data.num_data_points() - 1, data.data(), data.num_data_points() - 1) + 1 / params.cost;
////
////    // create C-SVM: must be done using the mock class, since plssvm::detail::gpu_csvm::calculate_w is protected
////    const mock_csvm_type svm = util::construct_from_tuple<mock_csvm_type>(params, TypeParam::additional_arguments);
////
...
//                             generate_q, calculate_w, device_reduction,
//                             select_num_used_devices, setup_data_on_device_minimal, num_available_devices);  // run_device_kernel, setup_data_on_device,
//// clang-format on
//
////*************************************************************************************************************************************//
////                                                         GPUCSVM DeathTests                                                          //
...
//                             generate_q, calculate_w, run_device_kernel, device_reduction, setup_data_on_device);  // select_num_used_devices
//// clang-format on
