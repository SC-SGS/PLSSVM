...
void init_regression_model(py::module_ &m) {
    ...
    py::class_<regression_model_wrapper>(m, "RegressionModel")
        ...
        .def("support_vectors", [](const regression_model_wrapper &self) { return std::visit([](auto &&model) { return plssvm::bindings::python::util::matrix_to_pyarray(model.support_vectors()); }, self.model); }, "the support vectors (note: all training points become support vectors for LSSVMs)")
        // clang-format off
