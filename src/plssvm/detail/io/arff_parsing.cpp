...
void read_arff_data(file_reader &reader, std::size_t header, std::size_t num_features, std::size_t max_size, bool has_label, std::shared_ptr<std::vector<std::vector<real_type>>> &X_ptr, std::shared_ptr<std::vector<real_type>> &y_ptr) {
    ...
    #pragma omp parallel default(none) shared(reader, X_ptr, y_ptr, parallel_exception) firstprivate(header, num_features, max_size, has_label)
    {
        #pragma omp for
        for (std::size_t i = 0; i < X_ptr->size(); ++i) {
                ...
                if (detail::starts_with(line, '{')) {
                    ...
                    while (true) {
                        ...
                        if (index == max_size - 1 && has_label) {
                            is_class_set = true;
                            (*y_ptr)[i] = plssvm::operators::sign(detail::convert_to<real_type, invalid_file_format_exception>(line.substr(pos)));
                        } else {
                    ...
                    for (std::size_t j = 0; j < max_size - 1; ++j) {
                        ...
                        (*X_ptr)[i][j] = detail::convert_to<real_type, invalid_file_format_exception>(line.substr(pos, next_pos - pos));
                        pos = next_pos + 1;
                    }
                    ...
                    if (has_label) {
                        ...
                        (*X_ptr)[i][num_features - 1] = detail::convert_to<real_type, invalid_file_format_exception>(line.substr(pos));
                    }
...
template void read_arff_data(file_reader &, std::size_t, std::size_t, std::size_t, bool, std::shared_ptr<std::vector<std::vector<float>>> &, std::shared_ptr<std::vector<float>> &);
template void read_arff_data(file_reader &, std::size_t, std::size_t, std::size_t, bool, std::shared_ptr<std::vector<std::vector<double>>> &, std::shared_ptr<std::vector<double>> &);

void write_arff_header(fmt::ostream &out, const std::size_t num_features, const bool has_labels) {
...
void write_arff_data(fmt::ostream &out, const std::shared_ptr<std::vector<std::vector<real_type>>> &X_ptr, const std::shared_ptr<std::vector<real_type>> &y_ptr) {
    ...
    // output with label
    for (std::size_t i = 0; i < X_ptr->size(); ++i) {
        out.print("{:e},{}\n", fmt::join((*X_ptr)[i], ","), (*y_ptr)[i]);
    }
}

template void write_arff_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<float>>> &, const std::shared_ptr<std::vector<float>> &);
template void write_arff_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<double>>> &, const std::shared_ptr<std::vector<double>> &);
...
template void write_arff_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<float>>> &);
template void write_arff_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<double>>> &);
