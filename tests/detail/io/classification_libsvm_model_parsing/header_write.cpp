 ...
 *
 * @brief Tests for writing a LIBSVM classification model file header.
 */
...
#include "tests/types_to_test.hpp"  // util::classification_label_type_gtest
...
TYPED_TEST(LIBSVMClassificationModelHeaderWrite, WriteSigmoid) {
    ...
    // check the written data
    ASSERT_EQ(reader.num_lines(), 10);  // the LIBSVM header
    EXPECT_EQ(reader.line(0), "svm_type c_svc");
    EXPECT_EQ(reader.line(1), "kernel_type sigmoid");
    EXPECT_EQ(reader.line(2), "gamma 0.4");
    EXPECT_EQ(reader.line(3), "coef0 4.4");
    EXPECT_EQ(reader.line(4), fmt::format("nr_class {}", distinct_label.size()));
    EXPECT_EQ(reader.line(5), fmt::format("label {}", fmt::join(distinct_label, " ")));
    EXPECT_EQ(reader.line(6), fmt::format("total_sv {}", data.num_rows()));
    EXPECT_EQ(reader.line(7), fmt::format("nr_sv {}", fmt::join(num_sv, " ")));
    EXPECT_EQ(reader.line(8), fmt::format("rho {:.10e}", fmt::join(rho, " ")));
...
template <typename T>
class LIBSVMClassificationModelHeaderWriteDeathTest : public LIBSVMClassificationModelHeaderWrite<T>,
...
TYPED_TEST(LIBSVMClassificationModelHeaderWriteDeathTest, WriteHeaderWithoutLabel) {
    ...
    const plssvm::classification_data_set<label_type> data_set{ std::vector<std::vector<plssvm::real_type>>{ { plssvm::real_type{ 0.0 } } } };
