...
void CPU_CSVM::learn(std::string &filename, std::string &output_filename) {
    ...
    if (info)
        std::clog << std::endl
                  << data.size() << " Datenpunkte mit Dimension " << num_features << " in " << std::chrono::duration_cast<std::chrono::milliseconds>(end_learn - end_parse).count() << " ms gelernt" << std::endl;
    ...
    if (info) {
        ...
        std::clog << data.size() << ", " << num_features << ", " << 0 << ", " << std::chrono::duration_cast<std::chrono::milliseconds>(end_learn - end_parse).count() << ", " << std::chrono::duration_cast<std::chrono::milliseconds>(end_write - end_learn).count() << std::endl;
    }
}
