...
class Layout : public ::testing::Test {
  protected:
    void SetUp() override {
        // save current verbose flag and disable verbose output to not clutter test terminal
        verbose_save = plssvm::verbose;
...
TYPED_TEST_SUITE(Layout, floating_point_types);
...
TYPED_TEST(LayoutDeathTest, empty_matrix) {
    ...
    std::vector<std::vector<TypeParam>> matrix{};
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::aos, matrix, 0, 0), "Matrix is empty!");
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::soa, matrix, 0, 0), "Matrix is empty!");
}
...
TYPED_TEST(LayoutDeathTest, too_many_data_points_to_transform) {
    ...
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::aos, matrix, 0, 3), "Number of data points to transform can not exceed matrix size!");
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::soa, matrix, 0, 3), "Number of data points to transform can not exceed matrix size!");
}
...
TYPED_TEST(LayoutDeathTest, different_number_of_features) {
    ...
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::aos, matrix, 0, 2), "Feature sizes mismatch! All features should have size 3.");
    EXPECT_DEATH(auto res = plssvm::detail::transform_to_layout(plssvm::detail::layout_type::soa, matrix, 0, 2), "Feature sizes mismatch! All features should have size 3.");
}
