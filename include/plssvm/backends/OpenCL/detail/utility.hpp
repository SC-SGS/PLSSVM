 ...
 * @details Manually caches the OpenCL JIT compiled code in the current `$TEMP` directory. The cached binaries are reused if:
 ...
 * @param[in] kernel_sources all source files needed to build the kernels
...
[[nodiscard]] std::vector<command_queue> create_command_queues(const std::vector<context> &contexts, target_platform target, const std::vector<std::string> &kernel_sources, const std::vector<std::pair<compute_kernel_name, std::string>> &kernel_names, bool print_info);
