    ...
    void operator()(::sycl::item<2> idx) const {
        ...
        if (feature_idx < num_features_ && class_idx < num_classes_) {
            ...
            for (unsigned long long sv = 0; sv < num_sv_; ++sv) {
                temp += alpha_d_[class_idx * num_sv_ + sv] * sv_d_[feature_idx * (num_sv_ + THREAD_BLOCK_PADDING) + sv];
            }
  ...
  public:
    ...
    void operator()(::sycl::item<3> idx) const {
        ...
        if (sv_idx < num_sv_ && predict_points_idx < num_predict_points_ && class_idx < num_classes_) {
            ...
            // perform dot product
            for (unsigned long long dim = 0; dim < num_features_; ++dim) {
                temp += sv_d_[dim * (num_sv_ + THREAD_BLOCK_PADDING) + sv_idx] * predict_points_d_[dim * (num_predict_points_ + THREAD_BLOCK_PADDING) + predict_points_idx];
            }
  ...
  public:
    ...
    void operator()(::sycl::item<3> idx) const {
        ...
        if (sv_idx < num_sv_ && predict_points_idx < num_predict_points_ && class_idx < num_classes_) {
            ...
            for (unsigned long long dim = 0; dim < num_features_; ++dim) {
                const real_type diff = sv_d_[dim * (num_sv_ + THREAD_BLOCK_PADDING) + sv_idx] - predict_points_d_[dim * (num_predict_points_ + THREAD_BLOCK_PADDING) + predict_points_idx];
                temp += diff * diff;
            }
