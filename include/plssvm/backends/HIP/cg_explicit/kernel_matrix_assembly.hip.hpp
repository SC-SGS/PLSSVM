...
__global__ void device_kernel_assembly_linear(real_type *ret, const real_type *data_d, const unsigned long long num_rows, const unsigned long long num_features, const real_type *q, const real_type QA_cost, const real_type cost) {
    ...
    __shared__ real_type data_cache_i[FEATURE_BLOCK_SIZE_OLD][THREAD_BLOCK_SIZE_OLD];
    __shared__ real_type data_cache_j[FEATURE_BLOCK_SIZE_OLD][THREAD_BLOCK_SIZE_OLD];

    if (blockIdx.x >= blockIdx.y) {
        real_type temp{ 0.0 };
        for (unsigned long long dim = 0; dim < num_features; dim += FEATURE_BLOCK_SIZE_OLD) {
            ...
            if (threadIdx.y < FEATURE_BLOCK_SIZE_OLD && dim + threadIdx.y < num_features) {
                if (i < num_rows) {
