...
struct device_ptr_test_type {
    ...
    using queue_type = std::unique_ptr<plssvm::@PLSSVM_SYCL_BACKEND_NAMESPACE_NAME@::detail::sycl::queue>;

    static const queue_type &default_queue() {
        static queue_type queue = std::make_unique<plssvm::@PLSSVM_SYCL_BACKEND_NAMESPACE_NAME@::detail::sycl::queue>();
        return queue;
    }
};
