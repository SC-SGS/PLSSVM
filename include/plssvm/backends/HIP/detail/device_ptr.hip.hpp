...
class device_ptr {
  ...
  public:
    ...
    /**
     * @brief Access the underlying HIP device pointer.
    ...
    /**
     * @brief Return the device associated with the wrapped HIP device pointer.
     * @return the device ID (`[[nodiscard]]`)
     */
    [[nodiscard]] int queue() const noexcept {
        return device_;
    }
     ...
     * @throws plssvm::hip::backend_exception if @p pos is greater or equal than device_ptr::size()
     */
     ...
     * @throws plssvm::hip::backend_exception if @p data_to_copy is too small to satisfy the memcpy
     */
    void memcpy_to_device(const std::vector<value_type> &data_to_copy);
    /**
     * @brief Memcpy up-to @p count many values from @p data_to_copy to the device starting at HIP device pointer position @p pos.
     * @details Copies `[p, rcount)` values where `rcount` is the smaller value of @p count and `device_ptr::size() - pos`.
     * @param[in] data_to_copy the data to copy onto the device
     * @param[in] pos the starting position for the copying in the HIP device pointer
     ...
     * @throws plssvm::hip::backend_exception if @p buffer is too small
     */
    void memcpy_to_host(std::vector<value_type> &buffer) const;
    /**
     * @brief Memcpy up-to @p count many values from the device starting at HIP device pointer position @p pos to the host buffer @p buffer.
     * @details Copies `[p, rcount)` values where `rcount` is the smaller value of @p count and `device_ptr::size() - pos`.
     * @param[out] buffer the buffer to copy the data to
     * @param[in] pos the starting position for the copying in the HIP device pointer
     * @param[in] count the number of elements to copy
     * @throws plssvm::hip::backend_exception if @p data_to_copy is too small
     */
