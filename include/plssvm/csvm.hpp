...
csvm<T>::csvm(kernel_type kernel, Args &&...named_args) {
    ...
    if constexpr (parser.has(gamma)) {
        ...
        static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(gamma))>, decltype(params_.gamma)>, "gamma must be convertible to a real_type!");
        // runtime check: the value may only be used with a specific kernel type
        if (kernel == kernel_type::linear) {
            print_warning_if_not_default(parser(gamma), "gamma");
        }
        // set value
        params_.gamma = static_cast<decltype(params_.gamma)>(parser(gamma));
    }
    if constexpr (parser.has(degree)) {
        // compile time check: the value must have the correct type
        static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(degree))>, decltype(params_.degree)>, "degree must be convertible to an int!");
        // runtime check: the value may only be used with a specific kernel type
        if (kernel == kernel_type::linear || kernel == kernel_type::rbf) {
            print_warning_if_not_default(parser(degree), "degree");
        }
        // set value
        params_.degree = static_cast<decltype(params_.degree)>(parser(degree));
    }
    if constexpr (parser.has(coef0)) {
        // compile time check: the value must have the correct type
        static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(coef0))>, decltype(params_.coef0)>, "coef0 must be convertible to a real_type!");
        // runtime check: the value may only be used with a specific kernel type
        if (kernel == kernel_type::linear || kernel == kernel_type::rbf) {
            print_warning_if_not_default(parser(coef0), "coef0");
        }
        // set value
        params_.coef0 = static_cast<decltype(params_.coef0)>(parser(coef0));
    }
    if constexpr (parser.has(cost)) {
        // compile time check: the value must have the correct type
        static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(cost))>, decltype(params_.cost)>, "cost must be convertible to a real_type!");
        // set value
        params_.cost = static_cast<decltype(params_.cost)>(parser(cost));
    }
...
auto csvm<T>::fit(const data_set<real_type, label_type> &data, Args &&...named_args) const -> model<real_type, label_type> {
    ...
    if constexpr (parser.has(epsilon)) {
        if constexpr (std::is_same_v<detail::remove_cvref_t<decltype(parser(epsilon))>, decltype(epsilon)>) {
            // a plssvm::default_value has been provided
            if (!parser(epsilon).is_default()) {
                // only override it if it doesn't hold the default value
                epsilon_val = parser(epsilon);
            }
        } else {
            // a normal value (not wrapped) has been provided
            // compile time check: the value must have the correct type
            static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(epsilon))>, real_type>, "epsilon must be convertible to a real_type!");
            // set value
            epsilon_val = static_cast<real_type>(parser(epsilon));
    ...
    if constexpr (parser.has(max_iter)) {
        if constexpr (std::is_same_v<detail::remove_cvref_t<decltype(parser(max_iter))>, decltype(max_iter_val)>) {
            // a plssvm::default_value has been provided
            if (!parser(max_iter).is_default()) {
                // only override it if it doesn't hold the default value
                max_iter_val = parser(max_iter);
            }
        } else {
            // a normal value (not wrapped) has been provided
            // compile time check: the value must have the correct type
            static_assert(std::is_convertible_v<detail::remove_cvref_t<decltype(parser(max_iter))>, size_type>, "max_iter must be convertible to a size_type!");
            // set value
            max_iter_val = static_cast<size_type>(parser(max_iter));
            // check if value makes sense
