    ...
    for (int j = 0; j < (dept + bloksize); j = j + bloksize) {
      ...
      for (int ii = 0; ii < bloksize && ii + i < dept; ++ii) {
        for (int jj = 0; jj < bloksize && jj + j < dept; ++jj) {
          if (ii + i > jj + j) {
            real_t temp = kernel_function(&data[ii + i][0], &data[jj + j][0], dim)
                - kernel_function(datlast, &data[ii + i][0], dim);
#pragma omp atomic
            r[jj + j] -= temp;
#pragma omp atomic
            r[ii + i] -= temp;
...
#pragma omp atomic
    r[i] -= kernel_function(&data[i][0], &data[i][0], dim) - kernel_function(datlast, &data[i][0], dim) + 1 / cost
        - (b.size() - i) * kernel_dat_and_cost;
...
#pragma omp parallel for schedule(dynamic, 8)
    for (int i = 0; i < b.size(); ++i) {
      real_t kernel_dat_and_cost = kernel_function(datlast, &data[i][0], dim) - QA_cost;
#pragma omp atomic
      Ad[i] += (kernel_function(&data[i][0], &data[i][0], dim) - kernel_function(datlast, &data[i][0], dim) + 1 / cost
          - kernel_dat_and_cost) * d[i];
      for (int j = 0; j < i; ++j) {
#pragma omp atomic
      ...
      for (int j = 0; j < (b.size() + bloksize); j += bloksize) {
        ...
        for (int ii = 0; ii < bloksize && ii + i < b.size(); ++ii) {
          for (int jj = 0; jj < bloksize && jj + j < b.size(); ++jj) {
            if (ii + i > jj + j) {
              real_t temp = kernel_function(&data[ii + i][0], &data[jj + j][0], dim)
                  - kernel_function(datlast, &data[jj + j][0], dim);
#pragma omp atomic
              r[jj + j] -= temp * x[ii + i];
#pragma omp atomic
              r[ii + i] -= temp * x[jj + j];
