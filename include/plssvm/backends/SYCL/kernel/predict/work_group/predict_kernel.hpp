...
class device_kernel_w_linear {
  public:
    ...
    device_kernel_w_linear(::sycl::handler &cgh, real_type *w, const real_type *alpha, const real_type *support_vectors, const std::size_t num_features, const std::size_t num_classes, const std::size_t num_sv, const std::size_t device_num_sv, const std::size_t device_sv_offset, const std::size_t grid_x_offset, const std::size_t grid_y_offset) :
        feature_cache_{ ::sycl::range<2>{ static_cast<std::size_t>(THREAD_BLOCK_SIZE), static_cast<std::size_t>(THREAD_BLOCK_SIZE) }, cgh },
        alpha_cache_{ ::sycl::range<2>{ static_cast<std::size_t>(THREAD_BLOCK_SIZE), static_cast<std::size_t>(THREAD_BLOCK_SIZE) }, cgh },
        w_{ w },
        alpha_{ alpha },
    ...
    void operator()(::sycl::nd_item<2> nd_idx) const {
...

        {
            ...
            for (std::size_t sv_block = 0; sv_block < device_num_sv_; sv_block += THREAD_BLOCK_SIZE_uz) {
                ...
                feature_cache_[local_id_0][local_id_1] = real_type{ 0.0 };
                alpha_cache_[local_id_0][local_id_1] = real_type{ 0.0 };

                // load data into local memory
                if (sv_block + threadIdx_x < device_num_sv_) {
                    if (global_feature_idx_linear < num_features_) {
                        feature_cache_[local_id_0][local_id_1] = support_vectors_[global_feature_idx_linear * device_num_sv_ + sv_block + threadIdx_x];  // SoA
                    }
                    if (global_class_idx_linear < num_classes_) {
                        alpha_cache_[local_id_0][local_id_1] = alpha_[global_class_idx_linear * num_sv_ + sv_block + device_sv_offset_ + threadIdx_x];  // AoS
                    }
                }
                ...
                for (unsigned sv = 0; sv < THREAD_BLOCK_SIZE; ++sv) {
                    temp += alpha_cache_[sv][local_id_0] * feature_cache_[sv][local_id_1];
                }
...
class device_kernel_predict_linear {
  public:
    ...
    device_kernel_predict_linear(::sycl::handler &cgh, real_type *prediction, const real_type *w, const real_type *rho, const real_type *predict_points, const std::size_t num_classes, const std::size_t num_predict_points, const std::size_t num_features, const std::size_t grid_x_offset, const std::size_t grid_y_offset) :
        pp_cache_{ ::sycl::range<2>{ static_cast<std::size_t>(THREAD_BLOCK_SIZE), static_cast<std::size_t>(THREAD_BLOCK_SIZE) }, cgh },
        w_cache_{ ::sycl::range<2>{ static_cast<std::size_t>(THREAD_BLOCK_SIZE), static_cast<std::size_t>(THREAD_BLOCK_SIZE) }, cgh },
    ...
    void operator()(::sycl::nd_item<2> nd_idx) const {
...

        {
            ...
            for (std::size_t feature_block = 0; feature_block < num_features_; feature_block += THREAD_BLOCK_SIZE_uz) {
                // zero-out local memory
                pp_cache_[local_id_0][local_id_1] = real_type{ 0.0 };
                w_cache_[local_id_0][local_id_1] = real_type{ 0.0 };

                // load data into local memory
                if (feature_block + threadIdx_x < num_features_) {
                    if (global_pp_idx_linear < num_predict_points_) {
                        pp_cache_[local_id_0][local_id_1] = predict_points_[(feature_block + threadIdx_x) * num_predict_points_ + global_pp_idx_linear];  // SoA
                    }
                    if (global_class_idx_linear < num_classes_) {
                        w_cache_[local_id_0][local_id_1] = w_[(feature_block + threadIdx_x) * num_classes_ + global_class_idx_linear];  // SoA
                    }
                }
                nd_idx.barrier();  // wait until all work-items loaded their part of the data
                ...
                for (unsigned feature = 0; feature < THREAD_BLOCK_SIZE; ++feature) {
                    temp += w_cache_[feature][local_id_0] * pp_cache_[feature][local_id_1];
                }
                nd_idx.barrier();  // wait until all work-items performed their part of the calculations
            }
        }
        ...
        if (global_pp_idx < num_predict_points_ && global_class_idx < num_classes_) {
            prediction_[global_pp_idx * num_classes_ + global_class_idx] = temp - rho_[global_class_idx];  // AoS
        }
    }
            ...
            for (std::size_t feature_block = 0; feature_block < num_features_; feature_block += THREAD_BLOCK_SIZE_uz) {
                ...
                if (feature_block + threadIdx_x < num_features_) {
                    ...
                    if (global_sv_idx_linear < num_sv_) {
                        sv_cache[local_id_0][local_id_1] = support_vectors_[(feature_block + threadIdx_x) * num_sv_ + global_sv_idx_linear];  // SoA
                    }
                }
                nd_idx.barrier();  // wait until all work-items loaded their part of the data
                ...
                for (unsigned feature = 0; feature < THREAD_BLOCK_SIZE; ++feature) {
                    temp += detail::feature_reduce<kernel_function>(sv_cache[feature][local_id_0],
...

        {
            ...
            for (std::size_t class_block = 0; class_block < num_classes_; class_block += THREAD_BLOCK_SIZE_uz) {
                ...
                for (unsigned class_idx = 0; class_idx < THREAD_BLOCK_SIZE; ++class_idx) {
                    out_cache[(class_idx + local_id_0) % THREAD_BLOCK_SIZE][local_id_1] += temp * alpha_cache[(class_idx + local_id_0) % THREAD_BLOCK_SIZE][local_id_0];
                    nd_idx.barrier();  // wait until all work-items performed their part of the calculations
                }
                ...
                if (class_block + threadIdx_x < num_classes_ && global_pp_idx < num_predict_points_) {
                    detail::atomic_op<real_type>{ prediction_[global_pp_idx * num_classes_ + class_block + threadIdx_x] } += out_cache[local_id_0][local_id_1];
                }
