...
bool read_libsvm_data(file_reader &reader, const std::size_t start, std::shared_ptr<std::vector<std::vector<real_type>>> &X_ptr, std::shared_ptr<std::vector<real_type>> &y_ptr) {
    std::exception_ptr parallel_exception;
    bool has_label = true;
    ...
    #pragma omp parallel default(none) shared(reader, start, X_ptr, y_ptr, parallel_exception, has_label)
    {
        ...
        for (typename std::vector<std::vector<real_type>>::size_type i = 0; i < X_ptr->size(); ++i) {
            ...
            try {
                ...
                if (first_colon >= pos) {
                    // get class or alpha
                    (*y_ptr)[i] = detail::convert_to<real_type, invalid_file_format_exception>(line.substr(0, pos));
                } else {
                    has_label = false;
                    pos = 0;
                }
...
template bool read_libsvm_data(file_reader &, const std::size_t, std::shared_ptr<std::vector<std::vector<float>>> &, std::shared_ptr<std::vector<float>> &);
template bool read_libsvm_data(file_reader &, const std::size_t, std::shared_ptr<std::vector<std::vector<double>>> &, std::shared_ptr<std::vector<double>> &);
...
void write_libsvm_data(fmt::ostream &out, const std::shared_ptr<std::vector<std::vector<real_type>>> &X_ptr, const std::shared_ptr<std::vector<real_type>> &y_ptr) {
    ...
    #pragma omp parallel default(none) shared(out, X_ptr, y_ptr, format_libsvm_line, count)
    {
        ...
        for (typename std::vector<real_type>::size_type i = 0; i < y_ptr->size(); ++i) {
            if ((*y_ptr)[i] >= 0) {
                format_libsvm_line(out_pos, (*y_ptr)[i], (*X_ptr)[i]);
            }
        }
        ...
        for (typename std::vector<real_type>::size_type i = 0; i < y_ptr->size(); ++i) {
            if ((*y_ptr)[i] < 0) {
                format_libsvm_line(out_neg, (*y_ptr)[i], (*X_ptr)[i]);
            }
        }
...
template void write_libsvm_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<float>>> &, const std::shared_ptr<std::vector<float>> &);
template void write_libsvm_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<double>>> &, const std::shared_ptr<std::vector<double>> &);
...
void write_libsvm_data(fmt::ostream &out, const std::shared_ptr<std::vector<std::vector<real_type>>> &X_ptr) {
    ...
    #pragma omp parallel default(none) shared(out, X_ptr, format_libsvm_line)
    {
        // all support vectors with class 1
...
template void write_libsvm_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<float>>> &);
template void write_libsvm_data(fmt::ostream &, const std::shared_ptr<std::vector<std::vector<double>>> &);
