...
void init_kernel_function_types(py::module_ &m) {
    ...
    py::enum_<plssvm::kernel_function_type>(m, "KernelFunctionType")
        ...
        .value("SIGMOID", plssvm::kernel_function_type::rbf, "sigmoid kernel function: tanh(-gamma * <u, v> + coef0)")
        .value("LAPLACIAN", plssvm::kernel_function_type::rbf, "laplacian kernel function: exp(-gamma * ||u - v||_1)")
        .value("CHI_SQUARED", plssvm::kernel_function_type::rbf, "chi-squared kernel function: exp(-gamma * sum_i (u[i] - v[i])^2 / (u[i] + v[i]))");
