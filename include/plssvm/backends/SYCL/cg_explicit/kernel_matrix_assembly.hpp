...
class device_kernel_assembly_linear {
  public:
    ...
    device_kernel_assembly_linear(::sycl::handler &cgh, real_type *ret, const real_type *data_d, const unsigned long long num_rows, const unsigned long long num_features, const real_type *q, const real_type QA_cost, const real_type cost) :
        data_cache_i_{ ::sycl::range<2>{ FEATURE_BLOCK_SIZE_OLD, THREAD_BLOCK_SIZE_OLD }, cgh }, data_cache_j_{ ::sycl::range<2>{ FEATURE_BLOCK_SIZE_OLD, THREAD_BLOCK_SIZE_OLD }, cgh },
    ...
    void operator()(::sycl::nd_item<2> nd_idx) const {
        ...
        if (nd_idx.get_group(0) >= nd_idx.get_group(1)) {
            real_type temp{ 0.0 };
            for (unsigned long long dim = 0; dim < num_features_; dim += FEATURE_BLOCK_SIZE_OLD) {
                ...
                if (nd_idx.get_local_id(0) < FEATURE_BLOCK_SIZE_OLD && dim + nd_idx.get_local_id(0) < num_features_) {
