...
template <typename T, typename S, std::enable_if_t<std::is_integral<S>::value, int> = 0 >
inline void cuda_memcpy(std::vector<T>& vec_dst, const std::vector<T>& vec_src, const S size, const cudaMemcpyKind kind) {
...
template <typename T>
inline void cuda_memcpy(T &dst,  const T* const &ptr_src, const cudaMemcpyKind kind = cudaMemcpyKind::cudaMemcpyDeviceToHost) {
  assert(kind == cudaMemcpyKind::cudaMemcpyDeviceToHost);
  cuda_memcpy(&dst, ptr_src, 1, kind);
}

template <typename T>
inline void cuda_memcpy(T* const &ptr_dst, const T& src, const cudaMemcpyKind kind = cudaMemcpyKind::cudaMemcpyHostToDevice) {
...
inline void cuda_memcpy(std::vector<T>& vec_dst, const D1 device_dest, std::vector<T>& vec_src, const D2 device_src, const cudaMemcpyKind kind) {
  assert( (vec_dst.size() == vec_src.size()) );
...
inline void cuda_memcpy(std::vector<T>& vec_dst, const D1 device_dest, const std::vector<T>& vec_src, const D2 device_src, const S size, const cudaMemcpyKind kind) {
  cuda_memcpy(vec_dst.data(), device_dest, vec_src.data(), device_src, size, kind);
}

template <typename T, typename S, typename D1, typename D2, std::enable_if_t<std::is_integral<S>::value, int> = 0, std::enable_if_t<std::is_integral<D1>::value, int> = 0, std::enable_if_t<std::is_integral<D2>::value, int> = 0>
inline void cuda_memcpy(T* const &ptr_dst, const D1 device_dest, const std::vector<T> &vec_src, const D2 device_src, const S size, const cudaMemcpyKind kind) {
  cuda_memcpy(ptr_dst, device_dest, vec_src.data(), device_src, size, kind);
}

template <typename T, typename S, typename D1, typename D2, std::enable_if_t<std::is_integral<S>::value, int> = 0, std::enable_if_t<std::is_integral<D1>::value, int> = 0, std::enable_if_t<std::is_integral<D2>::value, int> = 0>
inline void cuda_memcpy(std::vector<T>& vec_dst, const D1 device_dest, const T* const &ptr_src, const D2 device_src, const S size, const cudaMemcpyKind kind) {
  cuda_memcpy(vec_dst.data(), device_dest, ptr_src, device_src, size, kind);
}

template <typename T, typename S, typename D1, typename D2, std::enable_if_t<std::is_integral<S>::value, int> = 0, std::enable_if_t<std::is_integral<D1>::value, int> = 0, std::enable_if_t<std::is_integral<D2>::value, int> = 0>
inline void cuda_memcpy(T* const &ptr_dst, const D1 device_dest, const T* const &ptr_src, const D2 device_src, const S size) {
